...
class Features {
	...
	protected function getFeaturedHtml ($item) {
		$r = '<a href="' . $item['link'] . '">' .
			'<h3>' . $item['title'] . '</h3>' .
			// '<div class="image" style="background-image:url(' . $item['featureImage'] . ')"></div>' .
			'<div class="image">' .
	...
	protected function getAtomEntry ($item) {
		...
		$r .= '<entry>' .
			...
			'<summary type="html"><![CDATA[' .
				'<img src="' . $this->getLink($item['image']) . '" width="100" align="left" hspace="10"/>' .
				$item['content'] .
			']]></summary>';
		if (isset($item['tags'])) {
