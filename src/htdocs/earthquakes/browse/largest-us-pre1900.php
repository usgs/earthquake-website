<?php
  // Author: Lisa Wald
  // Contact: Paul Earle, Bruce Presgrave, Jim Dewey
  if (!isset($TEMPLATE)) {
  $TITLE = 'M7.5+ and Other Notable U.S. Earthquakes (or earthquakes affecting the U.S.) (Pre-1900)';
  $NAVIGATION = true;
  include 'template.inc.php';
  }
?>

<p>
  <strong>NOTE:</strong> All events are referenced using current political entities.
</p>

<div class="horizontal-scrolling">
  <table>

    <tr>
      <th> </th>
      <th>Mag</th>
      <th>Location</th>
      <th>Alternative Name</th>
      <th>Date (UTC)</th>
      <th>Time (UTC)</th>
      <th>Latitude</th>
      <th>Longitude</th>
      <th>References</th>
    </tr>

    <tr>
      <td><b>1.</b></td>
      <td>&#152;9</td>
      <td>Cascadia Subduction Zone</td>
      <td>1700-01-26</td>
      <td></td>
      <td></td>
      <td></td>
      <td></td>
      <td>Satake et al., 1996</td>
    </tr>
    <tr>
      <td><b>2.</b></td>
      <td>8.0</td>
      <td>Yakutat Bay, Alaska</td>
      <td></td>
      <td>1899-09-10</td>
      <td>21:41</td>
      <td>60.0&deg;N</td>
      <td>142.0&deg;W</td>
      <td></td>
    </tr>
    <tr>
      <td><b>3.</b></td>
      <td>7.9</td>
      <td>Near Cape Yakataga, Alaska</td>
      <td></td>
      <td>1899-09-04</td>
      <td>00:22</td>
      <td>60.0&deg;N</td>
      <td>142.0&deg;W</td>
      <td></td>
    </tr>
    <tr>
      <td><b>4.</b></td>
      <td>7.9</td>
      <td>Ka'u District, Island of Hawaii</td>
      <td>Hilea, SE Hawaii, Hawaii</td>
      <td>1868-04-03</td>
      <td>02:25</td>
      <td>19.2&deg;N</td>
      <td>155.5&deg;W</td>
      <td>Klein &amp; Wright, 2000</td>
    </tr>
    <tr>
      <td><b>5.</b></td>
      <td>7.9</td>
      <td><a href="/earthquakes/events/1857sca/">NE of San Luis Obispo</a></td>
      <td>The Great Fort Tejon, California Earthquake</td>
      <td>1857-01-09</td>
      <td>16:24</td>
      <td></td>
      <td></td>
      <td>Grant &amp; Sieh, 1993; Stein &amp; Hanks, 1998</td>
    </tr>
    <tr>
      <td><b>6.</b></td>
      <td>7.8</td>
      <td>Imperial Valley, California</td>
      <td></td>
      <td>1892-02-24</td>
      <td>07:20</td>
      <td></td>
      <td></td>
      <td></td>
    </tr>
    <tr>
      <td><b>7.</b></td>
      <td>7.5</td>
      <td><a href="http://earthquake.usgs.gov/earthquakes/states/events/1811-1812.php#december_16">New Madrid, Missouri</a></td>
      <td></td>
      <td>1811-12-16</td>
      <td>08:15</td>
      <td></td>
      <td></td>
      <td>Petersen et al., 2014</td>
    </tr>
    <tr>
      <td><b>8.</b></td>
      <td>7.5</td>
      <td><a href="http://earthquake.usgs.gov/earthquakes/states/events/1811-1812.php#december_16">New Madrid, Missouri</a></td>
      <td></td>
      <td>1812-01-23</td>
      <td>15:15</td>
      <td></td>
      <td></td>
      <td>Petersen et al., 2014</td>
    </tr>
    <tr>
      <td><b>9.<b></td>
      <td>7.4</td>
      <td>Bavispe, Sonora, Mexico</td>
      <td></td>
      <td>1887-05-03</td>
      <td>09:</td>
      <td>30.8&deg;N</td>
      <td>109.25&deg;W</td>
      <td>Stover &amp; Coffman, ???</td>
    </tr>
    <tr>
      <td><b>10.</b></td>
      <td>7.3</td>
      <td><a href="http://earthquake.usgs.gov/earthquakes/states/events/1811-1812.php#december_16">New Madrid, Missouri</a></td>
      <td></td>
      <td>1812-02-07</td>
      <td>09:45</td>
      <td></td>
      <td></td>
      <td>Petersen et al., 2014</td>
    </tr>
    <tr>
      <td><b>11.</b></td>
      <td>7.3</td>
      <td>Charleston, South Carolina</td>
      <td></td>
      <td>1886-09-01</td>
      <td>02:51</td>
      <td></td>
      <td></td>
      <td></td>
    </tr>
    <tr>
      <td><b>12.</b></td>
      <td>MMI ~ VII</td>
      <td>Cape Ann, Near Boston</td>
      <td></td>
      <td>11-18-1755</td>
      <td>09:11:35</td>
      <td></td>
      <td></td>
      <td></td>
    </tr>
    <tr>
      <td><b>13.</b></td>
      <td>~7</td>
      <td>St. Lawrence Valley</td>
      <td></td>
      <td>1663</td>
      <td></td>
      <td></td>
      <td></td>
      <td></td>
    </tr>


  </table>
</div>

<p class='alert warning'>
  Differing magnitudes and locations have been computed for some of these earthquakes; the values differ according to the methods and data used. The magnitudes, times, and locations in this list are considered to be the best available. The data in the list above may not currently match the data on the specific eventpages in the ComCat earthquake catalog since we are actively gathering catalog data from many sources and still flagging the authoritative data.
</p>

<h2>References</h2>
<ul class='referencelist'>
  <li>
    Grant, L.B. and K.E. Sieh, 1993, Stratigraphic evidence for seven meters of dextral slip on the San Andreas fault during the 1857 earthquake in the Carrizo Plain, Bull. Seism. Soc. Am., 83, 619-635.
  </li>
  <li>
    Klein, F.W. and T.L. Wright, 2000, Catalog of Hawaiian earthquakes, 1823-1959, USGS Prof. Paper 1623.
  </li>
  <li>
    Petersen, M.D., Moschetti, M.P., Powers, P.M., Mueller, C.S., Haller, K.M., Frankel, A.D., Zeng, Yuehua, Rezaeian, Sanaz, Harmsen, S.C., Boyd, O.S., Field, Ned, Chen, Rui, Rukstales, K.S., Luco, Nico, Wheeler, R.L., Williams, R.A., and Olsen, A.H., 2014, <a href="https://dx.doi.org/10.3133/ofr20141091">Documentation for the 2014 update of the United States national seismic hazard maps</a>: U.S. Geological Survey Open-File Report 2014–1091, 243 p.
  </li>
  <li>
    Satake, K., K. Shimazaki, Y. Tsuji, and K. Ueda, 1996, Time and size of a giant earthquake in Cascadia inferred from Japanese tsunami records of January 1700, Nature 379, 246-249.
  </li>
  <li>
    Stein, R.S. and T.C. Hanks, 1998, M&ge;6 earthquakes in southern California during the twentieth century: no evidence for a seismicity or moment deficit, Bull. Seismol. Soc. Am. Bull. 88, p. 635-652.
  </li>
  <li>
    Stover and Coffman, ???
  </li>
</ul>
