<?php
// Author: Lisa Wald
// Contact: Lisa Wald
  if (!isset($TEMPLATE)) {
    $TITLE = 'Earthquake Science Timeline';
    $NAVIGATION = true;
    $HEAD = '<link rel="stylesheet" href="eqscience-timeline.css"/>';
    include 'template.inc.php';
  }
?>

<div class="timeline">

<!-- *************** B.C. and A.D. **************** -->
<div class="year">
  <div class="inner">
    <span>B.C.</span>
  </div>
</div>

<ul class="days">
  <li class="day">
    <div class="events">
      <p>An earthquake and accompanying tsunami destroys and submerges the city of Helice, Greece.</p>
      <div class="date">373 B.C.</div>
    </div>
  </li>
</ul>

<div class="year">
  <div class="inner">
    <span>A.D.</span>
  </div>
</div>

<ul class="days">
  <li class="day">
    <div class="events">
      <p><a href="https://pubs.geoscienceworld.org/ssa/bssa/article/59/1/183/101553/the-early-history-of-seismometry-to-1900">First seismoscope</a> in Chang H&#234;ng, China.</p>
      <div class="date">132 A.D.</div>
    </div>
  </li>
</ul>



<!-- ****************** 1500 - 1799 ****************** -->
<div class="year">
  <div class="inner">
    <span>1500 - 1799</span>
  </div>
</div>

<ul class="days">

  <li class="day">
    <div class="events">
      <p>830,000 deaths are reported in Shaanxi, China earthquake.</p>
      <div class="date">1556</div>
    </div>
  </li>

  <li class="day">
    <div class="events">
      <p>Great Japanese Earthquake with a destructive tsunami and more than 5000 deaths.</p>
      <div class="date">1707</div>
    </div>
  </li>

  <li class="day">
    <div class="events">
      <p>Great Lisbon Earthquake and tsunami on November 1 almost completely destroys the city.</p>
      <div class="date">1755</div>
    </div>
  </li>

  <li class="day">
    <div class="events">
      <p>Five strong earthquakes on and after February 5 in Calibria, Italy. Scientific field investigation.</p>
      <div class="date">1783</div>
    </div>
  </li>
</ul>

<!-- ********************** 1800 ********************** -->
<div class="year">
  <div class="inner">
    <span>1800</span>
  </div>
</div>

<ul class="days">

  <li class="day">
    <div class="events">
      <p>Observed faulting after earthquake in Cutch, India on June 16 (Allah Bund).</p>
      <div class="date">1819</div>
    </div>
  </li>

  <li class="day">
    <div class="events">
      <p>First edition of Principles of Geology written by Charles Lyell.</p>
      <div class="date">1830</div>
    </div>
  </li>

  <li class="day">
    <div class="events">
      <p>Robert Mallet published his first general paper on earthquakes.</p>
      <div class="date">1846</div>
    </div>
  </li>

  <li class="day">
    <div class="events">
      <p>Early seismograph made by Karl Kreil. Faulting is observed after an earthquake in New Zealand on January 24.</p>
      <div class="date">1855</div>
    </div>
  </li>

  <li class="day">
    <div class="events">
      <p>Strike-slip offset is observed after <a href="https://www.usgs.gov/natural-hazards/earthquake-hazards/science/great-m79-1857-fort-tejon-earthquake">M7.9 Fort Tejon, California earthquake</a> on January 9. After an earthquake on December 16 in the Kingdom of Naples,  Robert Mallet, an Irish engineer, coins the term “seismology” during his field investigation of the earthquake.</p>
      <div class="date">1857</div>
    </div>
  </li>

  <li class="day">
    <div class="events">
      <p>Seismological observations begun at Manila with seismoscopes, seismographs installed 1881-1889.</p>
      <div class="date">1865</div>
    </div>
  </li>

  <li class="day">
    <div class="events">
      <p>Scarps and strike-slip offset observed after earthquake in Owens Valley, California on March 26.</p>
      <div class="date">1872</div>
    </div>
  </li>

  <li class="day">
    <div class="events">
      <p>A seismograph is developed in Japan by Alfre Ewing using a 7-meter long pendulum.</p>
      <div class="date">1879</div>
    </div>
  </li>

  <li class="day">
    <div class="events">
      <p>Seismological Society of Japan organized.</p>
      <div class="date">1880</div>
    </div>
  </li>

  <li class="day">
    <div class="events">
      <p>Explosion of Krakatoa volcano on August 27.</p>
      <div class="date">1883</div>
    </div>
  </li>

  <li class="day">
    <div class="events">
      <p>Rossi-Forel intensity scale is set up. Seismological service established in Manila.</p>
      <div class="date">1884</div>
    </div>
  </li>

  <li class="day">
    <div class="events">
      <p>Lord Rayleigh publishes a paper on the theory of "<a href="/learn/glossary/?term=Rayleigh wave">Rayleigh waves</a>."</p>
      <div class="date">1885</div>
    </div>
  </li>

  <li class="day">
    <div class="events">
      <p>Voigt investigates the elasticity of crystals, tensors are named and defined. Rudolph publishes the first paper on seaquakes. Seismological stations are established in California at Berkeley and Lick Observatory.</p>
      <div class="date">1887</div>
    </div>
  </li>

  <li class="day">
    <div class="events">
      <p>Amuri, New Zealand earthquake occurs on September 1, and strike-slip at the surface offsets fences.</p>
      <div class="date">1888</div>
    </div>
  </li>

  <li class="day">
    <div class="events">
      <p>First seismogram of a <a href="/learn/glossary/?term=teleseismic">teleseism</a> is identified by Ernst von Rebeur-Paschwitz from a recording of seismic waves on pendulums in Potsdam and Wilhemshaven, Germany.</p>
      <div class="date">1889</div>
    </div>
  </li>

  <li class="day">
    <div class="events">
      <p>Mino-Owari earthquake occurs in Japan with large <a href="/learn/glossary/?term=strike-slip">strike-slip</a> and <a href="/learn/glossary/?term=dip slip">dip slip</a>.</p>
      <div class="date">1891</div>
    </div>
  </li>

  <li class="day">
    <div class="events">
      <p>Japanese Imperial Earthquake Investigation Committee established. Sumatra earthquake on May 17, strike-slip shown by triangulation survey.</p>
      <div class="date">1892</div>
    </div>
  </li>

  <li class="day">
    <div class="events">
      <p>New seismographs developed by Vincentini in Padua, Italy and by Milne in England.</p>
      <div class="date">1894</div>
    </div>
  </li>

  <li class="day">
    <div class="events">
      <p>Seismology Committee of the British Association for the Advancement of Science is organized.</p>
      <div class="date">1896</div>
    </div>
  </li>

  <li class="day">
    <div class="events">
      <p>Great earthquake of Assam, India on June 12, investigation by Oldham.</p>
      <div class="date">1897</div>
    </div>
  </li>

  <li class="day">
    <div class="events">
      <p>Seismograph developed by Omori at Tokyo. Stations with Milne instruments initiated at Wellington, New Zealand and Batavia, Java.</p>
      <div class="date">1898</div>
    </div>
  </li>

  <li class="day">
    <div class="events">
      <p>Knott's equations for <a href="/learn/glossary/?term=reflection">reflection</a> and <a href="/learn/glossary/?term=refraction">refraction</a> coefficients of elastic waves. Great Alaskan earthquakes occur on September 3 and 10 with uplifts reaching 47 feet.</p>
      <div class="date">1899</div>
    </div>
  </li>
</ul>

<!-- ********************** 1900 ********************** -->
<div class="year">
  <div class="inner">
    <span>1900</span>
  </div>
</div>

<ul class="days">

  <li class="day">
    <div class="events">
      <p>A global network of 40 seismic stations is operational, sponsored by the British Association for the Advancement of Science and equipped with instruments designed by mining engineer John Milne.</p>
      <div class="date">1900</div>
    </div>
  </li>

  <li class="day">
    <div class="events">
      <p>Inverted pendulum seismograph is developed by Wiechert. Geophysical Institute founded at Gottingen.</p>
      <div class="date">1901</div>
    </div>
  </li>

  <li class="day">
    <div class="events">
      <p>International Seismological Association is organized.</p>
      <div class="date">1903</div>
    </div>
  </li>

  <li class="day">
    <div class="events">
      <p>Seismological station initiated at Uppsala, Sweden.</p>
      <div class="date">1904</div>
    </div>
  </li>

  <li class="day">
    <div class="events">
      <p>E.M. Anderson develops a theory that relates the direction of stress in the crust to three basic fault types – normal, strike-slip, and reverse faults.</p>
      <div class="date">1905</div>
    </div>
  </li>

  <li class="day">
    <div class="events">
      <p>Galitzin electromagnetic seismograph developed in Russia. Vertical-component seismograph set up by Straubel and Eppenstein at Jena, Germany. R.D. Oldham identifies three basic types of seismic waves: P waves, S waves, and L waves. A major deep-focus earthquake occurs under Japan on January 21. A Great earthquake occurs in Columbia and Ecuador on January 31. The Great 1906 San Francisco earthquake occurs on April 18, with a great extent of strike-slip faulting. A re-triangulation survey leads to elastic-rebound theory of earthquakes (see 1910).</p>
      <div class="date">1906</div>
    </div>
  </li>

  <li class="day">
    <div class="events">
      <p>A seismograph is developed by Mainka at Strassburg.</p>
      <div class="date">1907</div>
    </div>
  </li>

  <li class="day">
    <div class="events">
      <p>The Great Siberian meteorite fall occurs on June 30.</p>
      <div class="date">1908</div>
    </div>
  </li>

  <li class="day">
    <div class="events">
      <p>Andrija Mohorovici&ccaron;, a Serbian seismologist, identifies the boundary between the Earth’s crust and mantle layers by the different velocities of seismic waves through each layer after an earthquake near Zagreb, Croatia on October 8. It is called the "<a href="/learn/glossary/?term=Moho">Moho</a>". A seismological station is established at Riverview near Sydney, Australia.</p>
      <div class="date">1909</div>
    </div>
  </li>

  <li class="day">
    <div class="events">
      <p>Henry Fielding Reid proposes the <a href="https://www.usgs.gov/media/videos/elastic-rebound">elastic rebound</a> hypothesis to explain how earthquakes release accumulated energy when the fault slips, driven by plate tectonics. New instruments are installed at Berkeley.</p>
      <div class="date">1910</div>
    </div>
  </li>

  <li class="day">
    <div class="events">
      <p>Seismological Society of America is founded, and it begins publishing the Bulletin. New instruments are installed at Lick Observatory Station. The Galitzin vertical-component seismograph is developed in Russia. Theory of Love waves is introduced.</p>
      <div class="date">1911</div>
    </div>
  </li>

  <li class="day">
    <div class="events">
      <p>The radius of the <a href="/learn/glossary/?term=core">core</a> is determined by Gutenberg at Gottingen. Station established at La Paz, Bolivia.</p>
      <div class="date">1913</div>
    </div>
  </li>

  <li class="day">
    <div class="events">
      <p>Beno Gutenberg obtains the depth of the boundary between the Earth’s mantle and core.</p>
      <div class="date">1914</div>
    </div>
  </li>

  <li class="day">
    <div class="events">
      <p>Milne-Shaw seismograph is developed by J.J. Shaw in England.</p>
      <div class="date">1915</div>
    </div>
  </li>

  <li class="day">
    <div class="events">
      <p>First year covered by International Seismological Summary.</p>
      <div class="date">1918</div>
    </div>
  </li>

  <li class="day">
    <div class="events">
      <p>Zoeppritz equations for coefficients of reflection and refraction of elastic waves is published (posthumously).</p>
      <div class="date">1919</div>
    </div>
  </li>

  <li class="day">
    <div class="events">
      <p>Carnegie Institution of Washington, Advisory Committee in Seismology appointed. Explosion at Oppau, Germany on September 21.</p>
      <div class="date">1921</div>
    </div>
  </li>

  <li class="day">
    <div class="events">
      <p>Changes in the variation of latitude and in other geophysical elements.</p>
      <div class="date">1922</div>
    </div>
  </li>

  <li class="day">
    <div class="events">
      <p>The first global earthquake bulletins are made from internationally-shared seismic data. Wood-Anderson torsion seismometers began regular recording in Pasadena, California.</p>
      <div class="date">1923</div>
    </div>
  </li>

  <li class="day">
    <div class="events">
      <p>In response to the M7.9 1923 Kanto, earthquake, Japan establishes building codes for construction so that buildings can withstand shaking from large earthquakes. Nikiforov torsion seismographs in the service in the USSR.</p>
      <div class="date">1924</div>
    </div>
  </li>

  <li class="day">
    <div class="events">
      <p>Jesuit Seismological Association is organized with headquarters at St. Louis. Seismological work transferred from U.S. Weather Bureau to Coast and Geodetic Survey.</p>
      <div class="date">1925</div>
    </div>
  </li>

  <li class="day">
    <div class="events">
      <p>Geophysical Magazine (Tokyo) begins publication. Earthquake department of Pacific Board of Fire Underwriters established.</p>
      <div class="date">1926</div>
    </div>
  </li>

  <li class="day">
    <div class="events">
      <p>Seismological Laboratory at Pasadena occupied. The Tango earthquake in Japan occurs on March 7, is thoroughly investigated and found to have two fault traces.</p>
      <div class="date">1927</div>
    </div>
  </li>

  <li class="day">
    <div class="events">
      <p>K. Wadati publishes a paper on shallow and deep earthquakes.</p>
      <div class="date">1928</div>
    </div>
  </li>

  <li class="day">
    <div class="events">
      <p>New electromagnetic seismograph developed by Wenner. A New Zealand earthquake on June 17 sparks expansion of New Zealand seismology.</p>
      <div class="date">1929</div>
    </div>
  </li>

  <li class="day">
    <div class="events">
      <p>Variable-reluctance seismometer developed by Benioff in Pasadena, California. Permanent seismic stations are established around the world, and strong-motion seismometers (best at measuring seismic waves that damage buildings) are installed in some urban areas.</p>
      <div class="date">1930</div>
    </div>
  </li>

  <li class="day">
    <div class="events">
      <p>Benioff strain seismometer developed. U.S. Coast and Geodetic Survey initiates a program of strong-motion recording in California.</p>
      <div class="date">1932</div>
    </div>
  </li>

  <li class="day">
    <div class="events">
      <p>Great Sanriku earthquake and tsunami occurs on March 3. Permanent seismic stations are established around the world, and strong-motion seismometers (best at measuring seismic waves that damage buildings) are installed in some urban areas.</p>
      <div class="date">1933</div>
    </div>
  </li>

  <li class="day">
    <div class="events">
      <p>P'P' (P-prime P-prime) and related waves discovered at Pasadena.</p>
      <div class="date">1934</div>
    </div>
  </li>

  <li class="day">
    <div class="events">
      <p>Charles Richter develops the <a href="/learn/glossary/?term=Richter scale">Richter Scale</a> to measure earthquake size.</p>
      <div class="date">1935</div>
    </div>
  </li>

  <li class="day">
    <div class="events">
      <p>Inge Lehmann, a Danish seismologist, discovers the Earth’s inner core.</p>
      <div class="date">1936</div>
    </div>
  </li>

  <li class="day">
    <div class="events">
      <p>Istituto Nazionale di Geofisica established with headquarters at Rome.</p>
      <div class="date">1937</div>
    </div>
  </li>

  <li class="day">
    <div class="events">
      <p>On December 27 a destructive earthquake occurs in Turkey, the first of a series of extended and connected faulting.</p>
      <div class="date">1939</div>
    </div>
  </li>

  <li class="day">
    <div class="events">
      <p>T wave described and named by Linehan in Weston, Massachusetts. Harold Jeffreys and Keith Bullen create a model of the Earth’s interior based on seismic waves recorded around the world.  The seismic travel-time tables are then used to locate earthquakes for the next 50 years.</p>
      <div class="date">1940</div>
    </div>
  </li>

  <li class="day">
    <div class="events">
      <p>Seismicity of the Earth is published by Richter and Gutenberg, a classic book that documents that earthquakes are concentrated in narrow zones throughout the globe. These narrow zones are later recognized as the plate boundaries.</p>
      <div class="date">1941</div>
    </div>
  </li>

  <li class="day">
    <div class="events">
      <p>Annals de Geophysics begins publication.</p>
      <div class="date">1944</div>
    </div>
  </li>

  <li class="day">
    <div class="events">
      <p>On April 1, an M7.1 Aleutian Island earthquake generates a deadly tsunami that causes destruction on Hawaii. A seismic sea warning service is organized as a consequence. On July 24, an atomic bomb test at Bikini produces P waves that are recorded at distant stations.</p>
      <div class="date">1946</div>
    </div>
  </li>

  <li class="day">
    <div class="events">
      <p>Hegelian demolition on April 18.</p>
      <div class="date">1947</div>
    </div>
  </li>

  <li class="day">
    <div class="events">
      <p>The U.S. Coast and Geodetic Survey distributes the first earthquake probability map for the U.S. with 4 different hazard zones. C. A. Whitten, of the U.S. Coast and Geodetic Survey, publishes a paper on continuing horizontal <a href="/learn/glossary/?term=displacement">displacements</a> in California.</p>
      <div class="date">1948</div>
    </div>
  </li>

  <li class="day">
    <div class="events">
      <p>Annali di Geofisicia begins publication. A large quarry blast in Corona, California is recorded on August 6.</p>
      <div class="date">1949</div>
    </div>
  </li>

  <li class="day">
    <div class="events">
      <p>Japan deploys a warning system to stop trains when an earthquake is detected. On August 15, a Great earthquake occurs that affects Tibet and Assam.</p>
      <div class="date">1950</div>
    </div>
  </li>

  <li class="day">
    <div class="events">
      <p><a href="/learn/glossary/?term=strain">Strain</a>-release curves are developed by Benioff in Pasadena, California.</p>
      <div class="date">1951</div>
    </div>
  </li>

  <li class="day">
    <div class="events">
      <p>Lg waves are discovered by Ewing and Press at Columbia University, New York.</p>
      <div class="date">1952</div>
    </div>
  </li>

  <li class="day">
    <div class="events">
      <p>Pa and Sa waves are discovered by Caloi at Rome, and independently by Ewing and Press.</p>
      <div class="date">1953</div>
    </div>
  </li>

  <li class="day">
    <div class="events">
      <p>An earthquake occurs on March 29 at a very deep 640 kilometers under Spain.</p> On December 16, a major earthquake occurs in Nevada with large and extensive faulting.</p>
      <div class="date">1954</div>
    </div>
  </li>

  <li class="day">
    <div class="events">
      <p>Richter and Gutenberg develop formulas for calculating seismic energy, which were also used to determine the size of nuclear explosions.</p>
      <div class="date">1956</div>
    </div>
  </li>

  <li class="day">
    <div class="events">
      <p>The World Wide Standardized Seismographic Network (WWSSN) is started. It has high quality instruments able to record smaller earthquakes than before and able to get more accurate locations. The U.S. Uniform Building Code is modified to account for different building types and the level of seismic hazard in the region.</p>
      <div class="date">1961</div>
    </div>
  </li>

  <li class="day">
    <div class="events">
      <p>The <a href="/earthquakes/events/alaska1964/">M9.2 Alaskan earthquake</a> is the second largest in the 20th century, causing a tsunami 9 meters high and causing the planet to “ring like a bell”, enabling measurements of the Earth’s free oscillations, and providing definitive evidence of the subduction of the Pacific Plate beneath the North American plate beneath the continental margin of southern Alaska. The ground shaking in the M7.5 earthquake near Niigata, Japan causes liquefaction of the loose, water-saturated sediments, resulting in the tilting of some large apartment buildings.</p>
      <div class="date">1964</div>
    </div>
  </li>

  <li class="day">
    <div class="events">
      <p><a href="/learn/glossary/?term=plate%20tectonics">Plate tectonics</a> becomes a unified theory that brings together all the earthquake science up to that point and provides a basis upon which to build moving forward. New information shows that the amount of crust being subducted is balanced by the amount being produced at the mid-ocean ridges. A computer-generated map of earthquake locations around the world is used to delineate the major plate boundaries.</p>
      <div class="date">1965-68</div>
    </div>
  </li>

  <li class="day">
    <div class="events">
      <p>W.F. Brace and J.D. Byerlee demonstrate that an engineering phenomenon known as <a href="/learn/glossary/?term=stick-slip">stick-slip</a> also applies to geological materials, providing explanation for Reid’s elastic rebound theory. The National Earthquake Information Center (NEIC) is established in the U.S.</p>
      <div class="date">1966</div>
    </div>
  </li>

  <li class="day">
    <div class="events">
      <p>The M6.6 San Fernando earthquake near Los Angeles is recorded by 200+ strong-motion seismometers, showing for the first time a detailed picture of how ground shaking varies with time during an earthquake, and also varies greatly from place to place.</p>
      <div class="date">1971</div>
    </div>
  </li>

  <li class="day">
    <div class="events">
      <p>California passes the Alquist-Priolo Special Studies Act in 1972 that restricts the construction of new buildings for human occupancy across active fault zones. Land remote sensing imagery (Landsat) becomes available, allowing researchers to map faults in inaccessible areas.</p>
      <div class="date">1972</div>
    </div>
  </li>

  <li class="day">
    <div class="events">
      <p>An M7.3 earthquake near Haicheng, China is predicted about 24 hours in advance based on local increased seismic activity and changes in local water wells, and results in an evacuation of thousands unsafe buildings. T he earthquake destroys 90% of the city but with very few casualties, so the prediction is declared a success. A year later an M?.? earthquake happens near Tangshan, China with no prediction, killing more than 240,000 people, one of the highest death tolls from an earthquake.</p>
      <div class="date">1975</div>
    </div>
  </li>

  <li class="day">
    <div class="events">
      <p>Congress establishes the <a href="http://www.nehrp.gov/">National Earthquake Hazard Reduction Program (NEHRP)</a> to reduce the risks to life and property from future earthquakes in the U.S. and its territories. The Program is still in existence today.</p>
      <div class="date">1977</div>
    </div>
  </li>

  <li class="day">
    <div class="events">
      <p>Paleoseismology (the study of ancient earthquakes in the geologic record) is used to look at past earthquakes on the San Andreas Fault.</p>
      <div class="date">1978</div>
    </div>
  </li>

  <li class="day">
    <div class="events">
      <p>The <a href="https://www.usgs.gov/natural-hazards/earthquake-hazards/gsn-global-seismographic-network">Global Seismographic Network (GSN)</a> of broadband seismic stations is initiated with the founding of IRIS (Incorporated Research Institutes for Seismology). This network replaces the older WWSSN stations.</p>
      <div class="date">1984</div>
    </div>
  </li>

  <li class="day">
    <div class="events">
      <p>Japan installs continuously-recording GPS (Global Positioning System) stations for measuring tectonic deformation. With GPS satellites, these can be used for very precise locations of any point on Earth’s surface.</p>
      <div class="date">1988</div>
    </div>
  </li>

  <li class="day">
    <div class="events">
      <p><a href="https://www.usgs.gov/natural-hazards/earthquake-hazards/science/m-69-october-17-1989-loma-prieta-earthquake">M6.9 Loma Prieta, California earthquake</a> on October 17 causes $10.7 billion in losses.</p>
      <div class="date">1989</div>
    </div>
  </li>

  <li class="day">
    <div class="events">
      <p>InSAR (interferomatic synthetic aperture radar) technology is used to image earthquake faulting with more detail than ever possible before.  InSAR measures deformation by comparing reflected radar waves recorded on successive passes of a satellite from nearly identical positions.</p>
      <div class="date">1992</div>
    </div>
  </li>

  <li class="day">
    <div class="events">
      <p>M6.7 Northridge, California earthquake on January 17 causes $64.0 billion in losses.</p>
      <div class="date">1989</div>
    </div>
  </li>

  <li class="day">
    <div class="events">
      <p>An M6.9 directly under Kobe, Japan kills  more than 5500 people and destroys more than 100,000 buildings, causing roughly $200 billion in direct economic losses.</p>
      <div class="date">1995</div>
    </div>
  </li>

  <li class="day">
    <div class="events">
      <p>The USGS publishes a new series of national seismic hazard maps with contours showing 3 hazard levels for 4 different types of shaking (different frequencies).</p>
      <div class="date">1996</div>
    </div>
  </li>

  <li class="day">
    <div class="events">
      <p>Computer simulations and denser seismic networks provide the ability to simulate pictures of how the rupture occurs on the faults during the earthquakes. The first global seismic hazard map is produced.</p>
      <div class="date">1999</div>
    </div>
  </li>

</ul>

<!-- ********************** 2000 ********************** -->
<div class="year">
  <div class="inner">
    <span>2000</span>
  </div>
</div>

<ul class="days">

  <li class="day">
    <div class="events">
      <p>The M9.1 Great Sumatra earthquake and tsunami occurs, Congress increases earthquake funding.</p>
      <div class="date">2004</div>
    </div>
  </li>


  <li class="day">
    <div class="events">
      <p>The <a href="https://www.usgs.gov/natural-hazards/earthquake-hazards/national-earthquake-information-center-neic">National Earthquake Information Center (NEIC)</a> becomes a 24x7 operation.</p>
      <div class="date">2005</div>
    </div>
  </li>

  <li class="day">
    <div class="events">
      <p>The <a href="https://www.usgs.gov/natural-hazards/earthquake-hazards/anss-advanced-national-seismic-system">Advanced National Seismic System (ANSS)</a> network completes installment of 100th station.</p>
      <div class="date">2006</div>
    </div>
  </li>

<!--  NEED the DATE for this event
  <li class="day">
    <div class="events">
      <p>The IRIS Data Center is established, a database of earthquake sources and ground motions.</p>
      <div class="date">????</div>
    </div>
  </li>
  -->

  <li class="day">
    <div class="events">
      <p>QuakeML is released as the international standard for earthquake metadata exchange.</p>
      <div class="date">2007</div>
    </div>
  </li>

</ul>


<!-- This div ends the timeline -->
</div>


<h2>References</h2>

<ul>
  <li>
    Benz, H. (2017). <a href="http://srl.geoscienceworld.org/content/88/2B/457">Building a National Seismic Monitoring Center: NEIC from 2000 to the Present</a>, 88 (2B) 457-461.
  </li>
  <li>
    Leith, W. (2017). <a href="http://srl.geoscienceworld.org/content/88/4/943">NEHRP Turns 40</a>, Seismological Research Letters, 88 (4), 943-947.
  </li>
  <li>
    National Academies (2003). <a href="https://www.scribd.com/document/73632824/19514-Active-Earth-Poster":>Living on an Active Earth: Perspectives on Earthquake Science</a> (poster)
  </li>

  <li>
    Richter, Charles F. (1958). Elementary Seismology, W. H Freeman &amp; Company, Inc.
  </li>
</ul>


<!-- How to include an image
<li class="day">
  <div class="events">
    <div class="day__img">
      <img src="http://placehold.it/400x300" alt="" />
      <p class="caption">
        Lorem ipsum dolor sit amet.
      </p>
    </div>
    <div class="date">1999</div>
  </div>
</li>
-->
